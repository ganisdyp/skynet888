...
$character = $dataProvider->query->where(['id' => $_GET['id']])->one();
...
<div id="character-page" class="container">
    <nav class="mt-2">
        <ol class="breadcrumb smaller-90 mb-2">
            ...
            <li class="breadcrumb-item"><a
                        href="<?php echo Yii::$app->request->BaseUrl . '/site/character-category'; ?>"><?php echo Yii::t('common', 'Character'); ?></a>
            </li>
            <li class="breadcrumb-item bold"><a
                        href="/site/character-list?id=<?php echo $character->characterType->id; ?>"><?= $character->characterType->name; ?></a>
            </li>
            <li class="breadcrumb-item active"><?php echo $character->name; ?></li>
        </ol>
    </nav>
    ...
    <div class="row mt-3 mb-5">
        <div class="col-lg-5 col-12">
            <?php $media_type = $character->media_type;
            if ($media_type == 1) { ?>
                <iframe width="100%" height="280" src="<?= $character->main_photo; ?>" frameborder="0"
                        allow="autoplay; encrypted-media" allowfullscreen="allowfullscreen"></iframe>
            <?php } else { ?>
                <a href="<?= Yii::$app->request->BaseUrl; ?>/backend/uploads/character/<?= $character->main_photo; ?>"
                   data-lightbox="trip">
                    <div class="img-1by1 holder">
                        <img class="card-img-top img-responsive corner-0"
                             src="<?= Yii::$app->request->BaseUrl; ?>/backend/uploads/character/<?= $character->main_photo; ?>">
                    </div>
                </a>
            <?php } ?>
            <div class="row no-gutters mt-3">
                <?php
                $related_photos = $character->getCharacterPhotos()->where(['character_id' => $character->id])->all();
...
$table_photo = $photo->photo_url;
                    ...
                    <div class="col-3 pr-2 mb-2">
                        <a href="<?= Yii::$app->getHomeUrl() . 'backend/uploads/character/related_photo/' . $photo->photo_url; ?>"
                           data-lightbox="trip">
                            <div class="img-1by1 holder">
                                <?= Html::img(Yii::$app->getHomeUrl() . 'backend/uploads/character/related_photo/' . $photo->photo_url,
        ...
        <div class="col-lg-7 col-12">
            <p class="bigger-160 mb-1 text-purple font-weight-normal"><?php echo $character->name; ?></p>
            <div class="row mb-2">
                <div class="col-md-6 col-12">
                    <span><?= $character->project->name . " (" . $product->project->code . ")"; ?></span>
                </div>
