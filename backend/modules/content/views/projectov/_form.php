...
<?php $this->registerJs("
    $('.delete-button-photo').click(function() {
        var detail = $(this).closest('.projectov-profile');
        var updateType = detail.find('.update-type');
        if (updateType.val() === " . json_encode(ProjectovPhoto::UPDATE_TYPE_UPDATE) . ") {
            //marking the row for deletion
            updateType.val(" . json_encode(ProjectovPhoto::UPDATE_TYPE_DELETE) . ");
            detail.hide();
        } else {
        ...
        <div id="photo" class="tab-pane fade in active">
            <div class="row">
                <?php foreach ($modelDetails as $i => $modelDetail) : ?>
                    <div class="projectov-profile projectov-profile-<?= $i ?>" style="margin-top: .75rem;">
                        <div class="col-md-5 col-xs-10">
                            <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                            <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>
                            <?php //echo $form->field($modelDetail, "[$i]projectov_photo")->fileInput()
                            //
                            echo $form->field($modelDetail, "[$i]projectov_photo")->widget(FileInput::classname(), [
                                'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                    'showUpload' => false,
                                    'initialPreview' => [
                                        [Yii::$app->request->BaseUrl."/uploads/projectov/related_photo/$modelDetail->photo_url"]
                                    ],
                        ...
                        <div class="col-md-1 col-xs-2">
                            <div style="margin-top: 2rem;">
                                <?= Html::button('x', ['class' => 'delete-button-photo btn btn-danger', 'data-target' => "projectov-profile-$i"]) ?>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
