...
use common\models\EnvironmentTypeLang;
...
<div class="environment-form">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <?= $form->field($model, 'environment_type_id')->dropDownList(ArrayHelper::map(EnvironmentTypeLang::find()->all(), 'environment_type_id', 'name'), ['prompt' => '- Select -'])->label('Environment Category') ?>
        </div>
        <div class="col-md-6">
    ...
    <?php foreach ($modelDetails as $i => $modelDetail) : ?>
        <div class="row environment-profile environment-profile-<?= $i ?>">
            <div class="col-md-10">
                <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>

                <?= $form->field($modelDetail, "[$i]environment_photo")->fileInput() ?>
            </div>
            <div class="col-md-2">
                <?= Html::button('x', ['class' => 'delete-button btn btn-danger', 'data-target' => "environment-profile-$i"]) ?>
            </div>
        </div>
    <?php endforeach; ?>
