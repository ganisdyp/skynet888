...
<?php $this->registerJs("
    $('.delete-button-photo').click(function() {
        var detail = $(this).closest('.environment-profile');
        var updateType = detail.find('.update-type');
        if (updateType.val() === " . json_encode(EnvironmentPhoto::UPDATE_TYPE_UPDATE) . ") {
            //marking the row for deletion
            updateType.val(" . json_encode(EnvironmentPhoto::UPDATE_TYPE_DELETE) . ");
            detail.hide();
        } else {
...
<div class="environment-form">
                    ...
                    <div id="image-box" class="col-md-12">
                        ...
                        echo $form->field($model, 'main_photo_file')->widget(FileInput::classname(), [
                            'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                'showUpload' => false,
                                'initialPreview' => [
                                    ["http://www.safeboxasia.com/backend/uploads/environment/$model->main_photo"]
                                ],
            ...
            <div id="photo" class="tab-pane fade in active">
                <div class="row">
                    <?php foreach ($modelDetails as $i => $modelDetail) : ?>
                        <div class="environment-profile environment-profile-<?= $i ?>" style="margin-top: .75rem;">
                            <div class="col-md-5 col-xs-10">
                                <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                                <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>
                                <?php //echo $form->field($modelDetail, "[$i]environment_photo")->fileInput()
                                //
                                echo $form->field($modelDetail, "[$i]environment_photo")->widget(FileInput::classname(), [
                                    'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                        'showUpload' => false,
                                        'initialPreview' => [
                                            ["http://www.safeboxasia.com/backend/uploads/environment/related_photo/$modelDetail->photo_url"]
                                        ],
                                        'initialPreviewAsData' => true,
                            ...
                            <div class="col-md-1 col-xs-2">
                                <div style="margin-top: 2rem;">
                                    <?= Html::button('x', ['class' => 'delete-button-photo btn btn-danger', 'data-target' => "environment-profile-$i"]) ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
  ...
  $(\'#media-selector\').change(function(){
    ...
    if(isImage){
     $(\'#video-box\').hide();
   $(\'#image-box\').show();
    $(\'#environment-media_type\').val(2);
  
    }else{
     $(\'#image-box\').hide();
   $(\'#video-box\').show();
     $(\'#environment-media_type\').val(1);
