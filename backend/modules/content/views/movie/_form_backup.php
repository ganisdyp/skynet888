...
use common\models\MovieTypeLang;
...
<div class="movie-form">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            if ($model->isNewRecord) {
                ...
                echo "<source src='" . Yii::$app->getHomeUrl() . 'uploads/movie/' . $model->main_photo . "'  type='video/mp4'>";
                echo "</video>";
            } ?>
            <?= $form->field($model, 'main_photo_file')->fileInput() ?>
        </div>
        <div class="col-md-6">
            <?= $form->field($model, 'movie_type_id')->dropDownList(ArrayHelper::map(MovieTypeLang::find()->all(), 'movie_type_id', 'name'), ['prompt' => '- Select -'])->label('Movie Category') ?>
        </div>
        <div class="col-md-6">
    ...
    <?php foreach ($modelDetails as $i => $modelDetail) : ?>
        <div class="row movie-profile movie-profile-<?= $i ?>">
            <div class="col-md-10">
                <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>

                <?= $form->field($modelDetail, "[$i]movie_photo")->fileInput() ?>
            </div>
            <div class="col-md-2">
                <?= Html::button('x', ['class' => 'delete-button btn btn-danger', 'data-target' => "movie-profile-$i"]) ?>
            </div>
        </div>
    <?php endforeach; ?>
