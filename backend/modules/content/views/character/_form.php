...
<?php $this->registerJs("
    $('.delete-button-photo').click(function() {
        var detail = $(this).closest('.character-profile');
        var updateType = detail.find('.update-type');
        if (updateType.val() === " . json_encode(CharacterPhoto::UPDATE_TYPE_UPDATE) . ") {
            //marking the row for deletion
            updateType.val(" . json_encode(CharacterPhoto::UPDATE_TYPE_DELETE) . ");
            detail.hide();
        } else {
...
<div class="character-form">
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="col-md-12">
                <br>
                <?= $form->field($model, 'project_id')->dropDownList(ArrayHelper::map(ProjectLang::find()->all(), 'project_id', 'name'), ['prompt' => '- Select -','onchange' => 'checkCharacterExistence(this.value)'])->label('Related Project') ?>
            </div>
        </div>
    </div>
    <div class="row">
            ...
            if ($model->isNewRecord) {
                ...
                <div class="col-md-6">
                    ...
                    <div id="image-box" class="col-md-12">
                        <br>
                        <?php
                        /* echo Html::img(Yii::$app->getHomeUrl() . 'uploads/character/' . $model->main_photo,
                    ...
                    <div id="image-box" class="col-md-12">
                        ...
                        echo $form->field($model, 'main_photo_file')->widget(FileInput::classname(), [
                            'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                'showUpload' => false,
                                'initialPreview' => [
                                    [Yii::$app->request->BaseUrl."/uploads/character/$model->main_photo"]
                                ],
            ...
            <div id="photo" class="tab-pane fade in active">
                <div class="row">
                    <?php foreach ($modelDetails as $i => $modelDetail) : ?>
                        <div class="character-profile character-profile-<?= $i ?>" style="margin-top: .75rem;">
                            <div class="col-md-5 col-xs-10">
                                <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                                <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>
                                <?php //echo $form->field($modelDetail, "[$i]character_photo")->fileInput()
                                //
                                echo $form->field($modelDetail, "[$i]character_photo")->widget(FileInput::classname(), [
                                    'options' => ['accept' => 'image/*'], 'pluginOptions' => [
                                        'showUpload' => false,
                                        'initialPreview' => [
                                            [Yii::$app->request->BaseUrl."/uploads/character/related_photo/$modelDetail->photo_url"]
                                        ],
                                        'initialPreviewAsData' => true,
                            ...
                            <div class="col-md-1 col-xs-2">
                                <div style="margin-top: 2rem;">
                                    <?= Html::button('x', ['class' => 'delete-button-photo btn btn-danger', 'data-target' => "character-profile-$i"]) ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
  ...
  $(\'#media-selector\').change(function(){
    ...
    if(isImage){
     $(\'#video-box\').hide();
   $(\'#image-box\').show();
    $(\'#character-media_type\').val(2);
  
    }else{
     $(\'#image-box\').hide();
   $(\'#video-box\').show();
     $(\'#character-media_type\').val(1);
