...
use common\models\CharacterTypeLang;
...
<div class="character-form">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            if ($model->isNewRecord) {
                ...
                echo "<source src='" . Yii::$app->getHomeUrl() . 'uploads/character/' . $model->main_photo . "'  type='video/mp4'>";
                echo "</video>";
            } ?>
            <?= $form->field($model, 'main_photo_file')->fileInput() ?>
        </div>
        <div class="col-md-6">
            <?= $form->field($model, 'character_type_id')->dropDownList(ArrayHelper::map(CharacterTypeLang::find()->all(), 'character_type_id', 'name'), ['prompt' => '- Select -'])->label('Character Category') ?>
        </div>
        <div class="col-md-6">
    ...
    <?php foreach ($modelDetails as $i => $modelDetail) : ?>
        <div class="row character-profile character-profile-<?= $i ?>">
            <div class="col-md-10">
                <?= Html::activeHiddenInput($modelDetail, "[$i]id") ?>
                <?= Html::activeHiddenInput($modelDetail, "[$i]updateType", ['class' => 'update-type']) ?>

                <?= $form->field($modelDetail, "[$i]character_photo")->fileInput() ?>
            </div>
            <div class="col-md-2">
                <?= Html::button('x', ['class' => 'delete-button btn btn-danger', 'data-target' => "character-profile-$i"]) ?>
            </div>
        </div>
    <?php endforeach; ?>
